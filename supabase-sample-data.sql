-- Sample Data for AI Nutrition Companion Database
-- Run this after creating the main schema to populate with test data
...
('Strawberries', 'fruits', 32, 0.7, 8, 0.3, 2.0, '1 cup', 150, true),
...
('Bell Pepper', 'vegetables', 31, 1.0, 7, 0.3, 2.5, '1 medium pepper', 120, true),
...
('Whole Wheat Bread', 'grains', 247, 13, 41, 4.2, 7.0, '1 slice', 28, true),
...
('Cheddar Cheese', 'dairy', 403, 25, 1.3, 33, 0, '1 oz', 28, true),

-- Beverages
('Water', 'beverages', 0, 0, 0, 0, 0, '1 cup', 240, true),
('Green Tea', 'beverages', 1, 0, 0, 0, 0, '1 cup', 240, true);

-- Sample User Profile (you can modify this with actual test data)
...
-- Get the meal entry ID for food items (in a real scenario, you'd use the actual ID)
...
('test_user_123', 'Excellent choice! That''s a nutritious breakfast with good fiber, healthy fats, and protein. I''ve logged that meal for you. The oatmeal provides sustained energy, banana gives you potassium, and almonds add healthy fats and protein.', 'assistant');

-- Sample Achievements
INSERT INTO user_achievements (user_id, achievement_type, achievement_name, achievement_description, target_value, current_value, is_completed) VALUES
('test_user_123', 'meals_logged', 'First Meal', 'Log your first meal', 1, 1, true),
('test_user_123', 'streak', '3-Day Streak', 'Log meals for 3 consecutive days', 3, 1, false),
('test_user_123', 'water_intake', 'Hydration Hero', 'Drink 8 glasses of water in a day', 8, 4, false);

-- Sample Meal Plan
INSERT INTO meal_plans (user_id, plan_name, plan_description, start_date, end_date, total_days, plan_type, target_calories, target_protein, target_carbs, target_fat) VALUES
('test_user_123', '7-Day Healthy Living Plan', 'A balanced meal plan focused on whole foods and proper nutrition', CURRENT_DATE, CURRENT_DATE + INTERVAL '6 days', 7, 'custom', 2200, 165, 275, 73);

-- Sample Meal Plan Items
INSERT INTO meal_plan_items (meal_plan_id, day_number, meal_type, meal_name, meal_description, estimated_calories, estimated_protein, estimated_carbs, estimated_fat, ingredients, instructions, prep_time, cook_time) VALUES
((SELECT id FROM meal_plans WHERE user_id = 'test_user_123' LIMIT 1), 1, 'breakfast', 'Overnight Oats', 'Creamy overnight oats with berries and nuts', 350, 15, 45, 12, '{"1/2 cup rolled oats", "1/2 cup milk", "1 tbsp chia seeds", "1/2 cup mixed berries", "1 tbsp honey"}', 'Mix all ingredients in a jar, refrigerate overnight, enjoy cold', 5, 0),
((SELECT id FROM meal_plans WHERE user_id = 'test_user_123' LIMIT 1), 1, 'lunch', 'Quinoa Buddha Bowl', 'Nutritious bowl with quinoa, vegetables, and tahini dressing', 450, 18, 55, 16, '{"1 cup cooked quinoa", "1 cup roasted vegetables", "2 tbsp tahini", "1 tbsp lemon juice", "Mixed greens"}', 'Arrange quinoa and vegetables in bowl, drizzle with tahini dressing', 15, 25),
((SELECT id FROM meal_plans WHERE user_id = 'test_user_123' LIMIT 1), 1, 'dinner', 'Grilled Salmon with Sweet Potato', 'Omega-3 rich salmon with roasted sweet potato and steamed broccoli', 520, 35, 40, 22, '{"6 oz salmon fillet", "1 medium sweet potato", "1 cup broccoli", "1 tbsp olive oil", "Herbs and spices"}', 'Season salmon, grill for 4-5 minutes per side. Roast sweet potato at 400°F for 25 minutes. Steam broccoli for 5 minutes.', 10, 30);

-- Functions to help with data management
...
-- Function to get daily calorie needs (basic calculation)
